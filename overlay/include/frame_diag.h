...
/*
 * Diagnostic BEGIN-only mode.
 * framebufferBegin() is called for real so we exercise buffer acquisition.
 * The real pointer is logged but never exposed to the renderer: callers receive
 * a dummy buffer, so memset/drawing cannot touch the real framebuffer.
 * VSync wait and framebufferEnd() remain bypassed, so nothing is presented.
 */
static inline void *frame_diag_framebufferBegin(Framebuffer *fb, u32 *out_stride) {
    ...
    if (count < 8) frame_diag_log("BEGINONLY framebufferBegin enter", count);
    void *real_pixels = framebufferBegin(fb, out_stride);
    if (count < 8) {
        frame_diag_log("BEGINONLY framebufferBegin return", (long long)(uintptr_t)real_pixels);
        frame_diag_log("BEGINONLY stride", out_stride ? (long long)*out_stride : -1);
        frame_diag_log("BEGINONLY fb_size", fb ? (long long)fb->fb_size : -1);
        frame_diag_log("BEGINONLY returning dummy", (long long)(uintptr_t)dummy);
    }
    count++;
    return dummy;
}
...
static inline Result frame_diag_eventWait(Event *event, u64 timeout) {
    (void)event;
    static unsigned int count = 0;
    if (count < 8) frame_diag_log("BEGINONLY eventWait bypass", (long long)timeout);
    count++;
    return 0;
}
...
static inline void frame_diag_framebufferEnd(Framebuffer *fb) {
    (void)fb;
    static unsigned int count = 0;
    if (count < 8) frame_diag_log("BEGINONLY framebufferEnd bypass", count);
    count++;
}
